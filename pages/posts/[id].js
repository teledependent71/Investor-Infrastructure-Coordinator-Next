...
import postsPageInitialPathsBbeaeResource from '../../resources/posts-page-initial-paths-bbeae'
import postsPageInitialProps8d7fdResource from '../../resources/posts-page-initial-props-8d7fd'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Investor Infrastructure Coordinator</title>
          <meta
            property="og:title"
            content="Posts1 - Investor Infrastructure Coordinator"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsBbeaeResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps8d7fdResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
